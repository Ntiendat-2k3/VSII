...
const PropertyMap = () => {
  ...
  useEffect(() => {
    ...
    // Fetch master data (unit types, inquiry statuses, unit statuses)
    dispatch(fetchMasterData('UNIT_TYPE'));
    dispatch(fetchMasterData('INQUIRY_STATUS'));
    dispatch(fetchMasterData('UNIT_STATUS'));
  ...
  const handleLogin = useCallback(async (data: LoginFormData) => {
    setIsLoggingIn(true);
    try {
      const response = await axiosClient.post(AUTH_ENDPOINTS.LOGIN, data);
  ...
  const handleSelectUnit = useCallback(async (unitCode: string) => {
    try {
      const result = await propertyMapMutationApi.searchUnits(PROJECT_ID, unitCode);
